...
export async function supabaseRequest<T>(path: string, init: RequestInit = {}) {
  ...
  if (missing) throw new Error(`Supabase configuration is missing: ${missing}. Use literal values in .env.local, not process.env expressions, then restart npm run dev.`)
  ...
  if (!response.ok) {
    const detail = typeof payload === 'object' && payload !== null && 'message' in payload ? String(payload.message) : String(payload || response.statusText)
